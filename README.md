...
히어로 문구나 프로젝트 순서 정도가 아니라 **어떤 프로젝트를 아예 보여줄지**까지 다르게 하고 싶으면, `?c=코드` 대신 `site/` 아래에 별도 폴더(예: `site/nhn/`)를 만듭니다.

```
https://jiwonlee42.github.io/portfolio_web/nhn/
```

만드는 방법: `site/index.html`을 그 폴더로 복사한 뒤
- 상대경로를 한 단계 조정합니다: `assets/…` → `../assets/…`, `projects/…` → `../projects/…`
- 보여줄 프로젝트 카드만 남기고 순서를 정리합니다 (숨기는 프로젝트의 활동·수상 항목 속 링크도 함께 정리)
- 헤드라인과 소개 문단을 그 지원처에 맞게 다시 씁니다
- `<meta name="robots" content="noindex,nofollow">`를 head에 추가해 검색 결과에서 뺍니다
- `assets/`, `projects/`, `assets/data/*`는 그대로 재사용합니다(복사하지 않음). 그래서 GitHub 활동 그래프·블로그 글 목록은 자동으로 최신 상태를 따라가고, 프로젝트 상세 페이지를 고치면 회사별 페이지에도 그대로 반영됩니다.

이렇게 만든 폴더도 `?c=코드`와 마찬가지로 **공개**됩니다. 회사 이름이 URL에 그대로 남으니, 그 회사에만 보내는 링크로만 쓰고 다른 곳에 붙여넣지 않습니다.
